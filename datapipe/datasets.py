...
from stage1_utils.codeformer import CodeformerDataset, ResshiftOnlineDataset
...
def create_dataset(dataset_config):
    ...
    elif dataset_config['type'] == 'sr_paired':
        dataset = SRPairedDataset(**dataset_config['params'])
    elif dataset_config['type'] == 'resshift_online':
        dataset = ResshiftOnlineDataset(**dataset_config['params'])
    else:
...
class SRPairedDataset(Dataset):
    """
    Dataset for Paired Super-Resolution (Supports different input/output sizes).
    Reads LQ (Low Quality) and GT (Ground Truth) images from separate folders.
    """
    def __init__(self, lq_folder, gt_folder, out_size, im_exts=['png', 'jpg', 'jpeg']):
        super().__init__()
        self.lq_folder = lq_folder
        self.gt_folder = gt_folder
        
        # Scan files (Assumes filenames in both folders are identical)
        self.image_files = sorted([
            f for f in os.listdir(lq_folder) 
            if any(f.endswith(ext) for ext in im_exts)
        ])
    ...
    def __getitem__(self, index):
        ...
        gt_path = os.path.join(self.gt_folder, file_name)
        
        # 2. Read images using cv2
        # Read LQ image (e.g., 128x128)
        img_lq = cv2.imread(lq_path, cv2.IMREAD_COLOR)
        if img_lq is None: raise FileNotFoundError(f"Image not found: {lq_path}")
        # Convert BGR to RGB and normalize to [0, 1]
        img_lq = cv2.cvtColor(img_lq, cv2.COLOR_BGR2RGB).astype(np.float32) / 255.0
        
        # Read GT image (e.g., 512x512)
        img_gt = cv2.imread(gt_path, cv2.IMREAD_COLOR)
        if img_gt is None: raise FileNotFoundError(f"Image not found: {gt_path}")
