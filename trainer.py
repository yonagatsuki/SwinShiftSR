...
class TrainerBase:
    ...
    def prepare_data(self, data, dtype=torch.float32, phase='train'):
        # 增加调试打印 (如果还报错，这个打印会救命)
        print(f"DEBUG: Data Type: {type(data)}")
        if isinstance(data, list):
            # 如果不幸变成了 list，我们尝试挽救
            # 假设 list[0] 是 lq, list[1] 是 gt (或者反过来)
            # 这是一个比较危险的假设，但好过直接崩
            # 通常 tensor 会排在前面
            tensors = [x.cuda().to(dtype=dtype) for x in data if isinstance(x, torch.Tensor)]
            if len(tensors) == 2:
                # 盲猜第一个是 LQ，第二个是 GT (根据字典序 l 在 g 后面？不，通常随机)
                # 但我们可以根据 shape 判断吗？不行，都是 (B, C, H, W)
                # 这里的终极方案是：修改 Dataset 去掉 path，让它变回纯 Tensor 字典
                pass
    ...
    def logging_metric(self, metrics, tag, phase, add_global_step=False):
        ...
        if self.tf_logging:
            ...
            if isinstance(metrics, dict):
                # 如果是字典，说明有多个 loss (mse, lpips 等)
                # 这里的 metrics[key] 可能是一个数组 (记录了不同 timestep 的 loss)
                for key, value in metrics.items():
                    if isinstance(value, torch.Tensor) and value.numel() > 1:
                        # 如果是数组，我们记录它的平均值，或者记录第一个值
                        # 这里简单处理：记录平均值
