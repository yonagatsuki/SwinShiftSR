...
from .dataset_utils import load_file_list, center_crop_arr, random_crop_arr
from .common import instantiate_from_config
...
class ResshiftOnlineDataset(data.Dataset):
    def __init__(
        self,
        file_list: str,
        file_backend_cfg: Mapping[str, Any],
        ...
        noise_range: Sequence[float],
        jpeg_range: Sequence[int],
    ) -> "ResshiftOnlineDataset":
        super(ResshiftOnlineDataset, self).__init__()
        self.file_list = file_list
        self.image_files = load_file_list(file_list)
        self.file_backend = instantiate_from_config(file_backend_cfg)
        self.out_size = out_size # GT size (e.g., 512)
        self.crop_type = crop_type
        ...
        # 设定 LQ 的基准尺寸为 128
        # 逻辑: GT(512) -> Base(128) -> Degrade(128->small->128) -> LQ(128)
        self.lq_size = 128
    ...
    def load_gt_image(self, image_path: str, max_retry: int = 5) -> Optional[np.ndarray]:
        ...
        while image_bytes is None:
            ...
            image_bytes = self.file_backend.get(image_path)
            max_retry -= 1
            if image_bytes is None: time.sleep(0.5)
        image = Image.open(io.BytesIO(image_bytes)).convert("RGB")
        ...
        if self.crop_type == "center":
            image = center_crop_arr(image, self.out_size)
             ...
             if image.size != (self.out_size, self.out_size):
                 image = image.resize((self.out_size, self.out_size), Image.BICUBIC)
    ...
    def __getitem__(self, index: int) -> Dict[str, Union[np.ndarray, str]]:
        ...
        while img_gt is None:
            ...
            gt_path = image_file["image_path"]
            img_gt = self.load_gt_image(gt_path)
            if img_gt is None: index = random.randint(0, len(self) - 1)

        # 归一化到 [0, 1] (float32)
        img_gt = (img_gt / 255.0).astype(np.float32)
        
        # ============================================================
        # 逻辑: GT(512) -> Resize(128) -> Codeformer Degradation -> LQ(128)
        # ============================================================

        # 1. 预处理: 将 GT(512) 调整到 128 作为退化的基准图
        img_lq_base = cv2.resize(img_gt, (self.lq_size, self.lq_size), interpolation=cv2.INTER_LINEAR)
        h, w, _ = img_lq_base.shape # h=128, w=128

        # 2. 开始执行 CodeformerDataset 中的退化逻辑
        ...
        img_lq = cv2.filter2D(img_lq_base, -1, kernel)
        
        # --- Random Downsample (随机下采样) ---
        scale = np.random.uniform(self.downsample_range[0], self.downsample_range[1])
        img_lq = cv2.resize(
            img_lq, (int(w // scale), int(h // scale)), interpolation=cv2.INTER_LINEAR
        )
        ...
        # --- Resize back to original size (拉回 128) ---
        # 将退化后的小图拉回到 128x128
        img_lq = cv2.resize(img_lq, (w, h), interpolation=cv2.INTER_LINEAR)

        # ============================================================

        # 3. 转换范围到 [-1, 1] 并转为 Tensor
