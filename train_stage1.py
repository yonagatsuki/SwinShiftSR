...
from stage1_utils.common import instantiate_from_config, calculate_psnr_pt, to
...
def main(args) -> None:
    ...
    if accelerator.is_local_main_process:
        ...
        log_file = os.path.join(exp_dir, "training_records.log")
        logging.basicConfig(
            filename=log_file,
            filemode='a', # 'a' 表示追加模式
        ...
        logging.info(f"Training Started. Experiment Dir: {exp_dir}")
    ...
    if cfg.train.resume:
        ...
        # 尝试从文件名提取步数 (假设文件名是 "150000.pt")
        try:
            # 获取文件名 "150000.pt" -> 去掉后缀 -> 转整数
            resume_step = int(os.path.basename(cfg.train.resume).split('.')[0])
            global_step = resume_step
            if accelerator.is_local_main_process:
                print(f"Resuming step counter from: {global_step}")
        except Exception as e:
            print(f"Warning: Could not parse step from filename. Step starts at 0. Error: {e}")
    ...
    while global_step < max_steps:
        ...
        for batch in loader:
            ...
            # === 【新增】计算 Mean Loss (仅用于显示) ===
            with torch.no_grad():
                loss_mean_val = F.mse_loss(pred, gt, reduction="mean").item()
            opt.zero_grad()
            ...
            step_loss_mean.append(loss_mean_val) 
            
            epoch_loss.append(loss.item())
            pbar.update(1)
            
            pbar.set_description(
                f"Epoch: {epoch:04d}, Step: {global_step:07d}, Sum: {loss.item():.1f}, Mean: {loss_mean_val:.6f}"
            )

            # Log loss values:
            if global_step % cfg.train.log_every == 0:
                # Gather values from all processes (Sum Loss)
                avg_loss = (
                    accelerator.gather(
                ...
                avg_loss_mean = (
                    accelerator.gather(
                        torch.tensor(step_loss_mean, device=device).unsqueeze(0)
                    )
                    .mean()
                    .item()
                )
                step_loss_mean.clear()

                
                if accelerator.is_local_main_process:
                    writer.add_scalar("train/loss_sum", avg_loss, global_step)
                    writer.add_scalar("train/loss_mean", avg_loss_mean, global_step) 
                    # === 【修改】写入日志文件 ===
                    log_msg = f"[Train] Step: {global_step:07d} | Sum: {avg_loss:.2f} | Mean: {avg_loss_mean:.6f}"
            ...
            if global_step % cfg.train.val_every == 0:
                ...
                if accelerator.is_local_main_process:
                    ...
                    logging.info(f"--------------------------------------------------")
                    logging.info(f"[Validation] Step: {global_step:07d}")
                    logging.info(f"  - PSNR : {avg_val_psnr:.4f}")
                    logging.info(f"  - LPIPS: {avg_val_lpips:.4f}")
                    logging.info(f"  - Loss : {avg_val_loss:.6f}")
                    logging.info(f"--------------------------------------------------")
